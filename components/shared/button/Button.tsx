...
function Button({
  ...
  if(btn == 'fill'){
    return (
      <button
        className={`duration-300 disabled:bg-gray-3 disabled:text-gray-4 ${theme === 'Primary' ? 'bg-primary text-white hover:bg-shade-2 selection:bg-shade-3' : theme === 'White' ? 'bg-tint-1 text-primary hover:bg-tint-2' : 'bg-gray-7 text-white hover:bg-gray-8'} rounded-sm md:rounded-md flex items-center justify-around relative ${btnSize}`}
    ...
    return (
      ...
      >
        {loading !== true ? <>
        {iconR && (
          <Image src={iconR} alt={iconR} width={iconSize} height={iconSize}  className={`${iconW} ${iconH} [position:unset]`} />
        )}
  
        {title}
  
        {iconL && (
          <Image src={iconL} alt={iconL} width={iconSize} height={iconSize} className={`${iconW} ${iconH} [position:unset]`} />
        )}
        </> : <>
          <div className='loader'></div>
        </>}
